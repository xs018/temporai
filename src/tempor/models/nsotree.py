...
class NSOTree(nn.Module):
    ...
    def normal_forward(self, init_layer, p=0, training=True):
        ...
        return self.output_fc(cur_embed)

    def forward(self, init_layer, time=None, p=0.4, alpha=None, anneal='none'):
        # print(init_layer.shape, time.shape)
        init_layer = torch.cat([init_layer, time], dim=-1) if time is not None else init_layer
        if self.net_type == 'locally_constant':
            self.relu_masks = []
            if len(init_layer.shape) == 4:
                bz = init_layer.shape[0]
            ...
            for i in range(self.num_layer):
                ...
                if self.drop_type == 'node_dropconnect':
                    ...
                    next_embed = torch.bmm(F.dropout(w, p=p, training=self.training), cur_embed.unsqueeze(-1)) + b.unsqueeze(-1)
                    next_embed = next_embed.squeeze(-1)
                else:
                    pass

                self.relu_masks.append( (next_embed > 0) )
                ...
                self.patterns.append(neur_deriv)
            ...
            return self.normal_forward(init_layer, p, self.training)
...
if __name__ == "__main__":
    ...
    model = NSOTree(input_dim=input_dim, output_dim=output_dim, hidden_dim=hidden_dim, num_layer=depth, num_back_layer=back_n, dense=True, drop_type=drop_type, net_type=net_type, approx=anneal).cuda()
    print(model)
    x = torch.randn(5, 10).cuda()
    leaf_output = model(x)
    # print(relu_masks)
    print(leaf_output.size())
